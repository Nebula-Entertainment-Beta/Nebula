 ...
 * @brief Semantic **actions** and **axes** built on top of `Input` (`Tasto`, mouse delta, scroll).
 *
 * **Owns:** `ActionMapping`, enums `Action` / `Axis`, `AxisBinding`, and `CameraSensitivity`.
 *
 * **Why:** Gameplay asks for “move forward” or “look X”, not raw scan codes, so bindings stay centralized.
 */
#pragma once
...
namespace Nebula
{
    ...
    struct AxisBinding
    {
        ...
        std::optional<Tasto> positiveKey;
        std::optional<Tasto> negativeKey;
        float scale = 1.0f;
    };
    ...
    public:
        ...
        void addActionBinding(Action action, Tasto key);
        void warnOnce(const std::string &message);

        void getCameraSensitivity(float &outLookSensitivityX, float &outLookSensitivityY,
                                  float &outZoomSensitivity, bool &outInvertLookY) const
        {
        ...
        void setCameraSensitivity(float lookSensitivityX, float lookSensitivityY, float zoomSensitivity,
                                  bool invertLookY)
        {
